...
class Estimation:
    ...
    def round(self, precision: int):
        ...
        if self.value == 0:
            return self.value
        self.value = round(self.value, precision - int(floor(log10(abs(self.value)))) - 1)
...
class AccuracyEstimation(Estimation):
    ...
    def __init__(self, value: float, success: bool = True):
        ...
        if not (0 <= value <= 100):
            raise ValueError(f'AccuracyEstimation value must be between 0 and 100, not {value}')
        if value == 0:
...
class AccelergyQuery():
    ...
    def __str__(self):
        ...
        if self.action_name:
            args_stringified = ', '.join([f'{k}={v}' for k, v in self.action_args.items()])
