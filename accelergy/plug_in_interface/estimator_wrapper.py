...
class CallableFunction:
    ...
    def __init__(self, function: Callable, logger: logging.Logger, force_name_override: str = None, 
        ...
        default_length = len(function.__defaults__) if function.__defaults__ is not None else 0
    ...
    def get_error_message_for_non_default_arg_match(
                self, kwags: dict, class_name: str = '') -> Optional[str]:
        for arg in self.non_default_args:
            if kwags.get(arg) is None:
                return f'Argument for {class_name}.{self.function_name} is missing: {arg}. ' \
                       f'Arguments provided: {", ".join(kwags.keys())}'
        return None
    
    def get_call_error_message(
                self, name: str, kwargs: dict, class_name: str = '') -> Optional[str]:
    ...
    def call(self, kwargs: dict, class_name: str = '', 
             call_function_on_object: object = None) -> Any:

        kwags_included = {
            k: v for k, v in kwargs.items() if k in self.non_default_args or k in self.default_args
        }
...
class EstimatorWrapper(AccelergyPlugIn):
    ...
    def __init__(self, estimator_cls: type, class_name: str):
        ...
        self.actions = [
            CallableFunction(getattr(estimator_cls, a), self.logger) for a in dir(estimator_cls) if
            getattr(getattr(estimator_cls, a), '_is_accelergy_action', False,)
        ]
        INFO(f'Added estimator {self.estimator_name} that estimates {self.class_name} with actions '
    ...
    def fail_missing(self, missing: str):
        raise AttributeError(f'Primitive component {self.class_name} '
                             f'must have {missing}')


    def is_class_supported(self, query: AccelergyQuery) -> bool:
        name_check = [self.class_name] if isinstance(self.class_name, str) else self.class_name
    ...
    def estimate_energy(self, query: AccelergyQuery) -> Estimation:
        ...
        estimation = Estimation.from_value(
            supported_actions[0].call(query.action_args, self.class_name, initialized_obj))
        move_queue_from_one_logger_to_another(initialized_obj.logger, self.logger)
        return estimation
...
def get_all_estimators_in_module(module: ModuleType) -> List[Estimator]:
    print(f'Getting all estimators in module {module}')
...
def check_for_valid_estimator_attrs(estimator: Estimator):
    ...
    name = estimator.name
    if not isinstance(name, str) and not (isinstance(name, list) and 
                                          all(isinstance(n, str) for n in name)):
        raise AttributeError(
    ...
    if getattr(estimator, 'percent_accuracy_0_to_100', None) is None:
        raise AttributeError(f'Estimator for {name} must have a "percent_accuracy_0_to_100" '
                             f'attribute.')
    percent_accuracy = estimator.percent_accuracy_0_to_100
    if not isinstance(percent_accuracy, Number):
        raise AttributeError(f'Estimator for {name} percent_accuracy_0_to_100 must be a '
                             f'number. It is currently a {type(percent_accuracy)}')
    if percent_accuracy < 0 or percent_accuracy > 100:
        raise AttributeError(f'Estimator for {name} percent_accuracy_0_to_100 must be '
