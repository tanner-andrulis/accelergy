...
class Action(object):
    ...
    def set_arguments(self, action_def_dict):
        if 'arguments' in action_def_dict:
            self._arguments = {}
            sorted_args = sorted(action_def_dict['arguments'].items(), key=lambda x: x[0])
            for arg_name, arg_range in sorted_args:
    ...
    def map_arg_range_bounds(arg_range_str, attributes_dict):
        ...
        try:
            ...
            v = parse_expression_for_arithmetic_new(split_sub_string[0], attributes_dict, 'index range expression', strings_allowed=False)
        ...
        try:
            ...
            v = parse_expression_for_arithmetic_new(split_sub_string[1], attributes_dict, 'index range expression', strings_allowed=False)
            if not isinstance(v, str):
                end_idx = v
            else:
                if split_sub_string[1] not in attributes_dict:
                    ERROR(f'ERROR: {split_sub_string[1]} is not a valid expression.')
