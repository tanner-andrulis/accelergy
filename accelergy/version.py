...
def check_input_parser_version(input_parser_version, input_file_type, input_file_path):
    ...
    if not versions_compatible(PARSER_VERSION, INPUT_VERSION):
        ERROR_CLEAN_EXIT(f'Input file {input_file_path} version {input_parser_version} is '
    ...
    if len(INPUT_FILE_VERSIONS) > 1:
        ...
        ERROR_CLEAN_EXIT(f'Input files of multiple versions detected. Input file versions are {INPUT_FILE_VERSIONS}. '
